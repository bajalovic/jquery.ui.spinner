    ...
    $.widget('ui.spinner', {
        ...
        _mouseWheel: function (e) {
            ...
            if (!self.options.disabled && self.focused && (focusCtrl === self)) {
                ...
                self._change();
                self._doSpin(((e.wheelDelta || -e.detail) > 0 ? 1 : -1) * self.options.mouseWheelStep);
                return false;
            }
        },
        ...
        _doSpin: function (step) {
            ...
            if(self.options.timeFormat) {
	            var dateStart = new Date("Jan 1, 1970" + " " + value );
	    		
	    		var newDateTS   = Date.parse(dateStart) + dateStart.setTime( step * 60000) ;
	    		var newDate = new Date(newDateTS);
	    		var tmpH = newDate.getHours().toString();
	    		var tmpM = newDate.getMinutes().toString();
	    		tmpH = ((tmpH.length == 1) ? '0' : '') + tmpH;
	    		tmpM = ((tmpM.length == 1) ? '0' : '') + tmpM;

	            self._setValue(tmpH + ":" +tmpM);
            } else {
            	if (value == null)
        ...
        _change: function () {
            ...
            if(self.options.timeFormat) {
	            ...
	            if(regex.test(value)) {
	            	...
	            	if(tmp_val[0] < 24 && tmp_val[1] < 60) {
		            	tmp_h = ((tmp_val[0].length == 1) ? '0' : '') + tmp_val[0];
		            	tmp_m = tmp_val[1] + ((tmp_val[1].length == 1) ? '0' : '');
		            	value = tmp_h + ":" + tmp_m;
	            	} else {
	            		value = self.curvalue;
	            	}
	            	...
	            	if(len == 0 || len > 4) {
	            		...
	            		case 2:
	            			...
	            			break;
	            		case 3:
	            			var tmp_m = value.substr(1, 3);
	            			var tmp_h = value.substr(0, 1);
	            			if(tmp_h <= 9 && tmp_m >= 0 && tmp_m < 60) {
	            				value = "0"+tmp_h+":"+tmp_m;
	            			} else {
	            				value = self.curvalue;
	            			}
	            			break;
	            		case 4:
	            			var tmp_m = value.substr(2, 4);
	            			var tmp_h = value.substr(0, 2);
	            			if(tmp_m >= 0 && tmp_m < 60 && tmp_h >= 0 && tmp_h <= 24) {
	            				value = tmp_h+":"+tmp_m;
	            			} else {
	            				value = self.curvalue;
	            			}
